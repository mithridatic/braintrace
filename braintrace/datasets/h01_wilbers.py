...
def rate_trap(voltage_mv, threshold_mv, scale, slope_mv):
    ...
    return scale * slope_mv * jnp.where(small, 1 + x/2 + x*x/12, regular)


def sodium_rates(voltage_mv, *, temperature_c=34.):
    ...
    tuple
        ...
        in ms. Temperature scales time constants, not steady states.
    """
...
def potassium_rates(voltage_mv, *, temperature_c=34.):
    ...
    tuple
        ...
        constant and fast-inactivation time constant in ms. Slow inactivation
        uses 450 ms and recovery 2789.7929428108187 ms in the source and must
        be selected using the slow gate state, separately from this function.
    """
    v = jnp.asarray(voltage_mv)

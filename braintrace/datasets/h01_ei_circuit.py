"""A two-cell H01 diagnostic circuit with explicit inferred synaptic wiring."""
import numpy as np
import braincell
...
def make_h01_ei_circuit(components, annotations, *, regions, region_basis,
                        currents_na=None, control="ei", excitatory_weight_us=.01,
                        inhibitory_weight_us=.02, delay_ms=.5, max_cv_length_um=10.):
    """Build reciprocal E/I wiring and matched projection-removal controls.
    ...
    currents_na : dict, optional
        Soma pulse amplitudes from 2 to 5 ms. Default E=1, I=0 nA.
    ...
    currents = {"E": 1., "I": 0.} if currents_na is None else dict(currents_na)
    if set(currents) != {"E", "I"}:
    ...
    evidence = {"cells": {}, "control": control, "inferred_contacts": [],
                ...
                "connectivity_basis": "Illustrative reciprocal wiring; no measured H01 partner mapping."}
    for role, source_tag in (("E", "pyramidal"), ("I", "interneuron")):
        ...
        cell.place(soma, Synapse("ExpSyn", name=incoming, e=reversal*u.mV, tau=tau*u.ms, weight=1.*u.uS))
        cell.place(soma, MechanismProbe(mechanism=incoming, field="g", name="synaptic_conductance"))
        site = restrict_spike_output(cell, soma)
        cell.place(AtLocation(*site["output_midpoint"]), StateProbe(field="v", name="output_voltage"))
        record["output_site"] = restrict_spike_output(cell, soma)
        record["synaptic_connectivity"] = "See circuit inferred_contacts; no measured connection asserted."

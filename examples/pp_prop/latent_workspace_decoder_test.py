...
def test_split_returns_the_shapes_the_existing_consumers_expect() -> None:
    ...
    np.testing.assert_array_equal(
        jnp.concatenate(
            (height, width, colors.reshape(2, -1)), axis=-1
        ),
        compact,
    )
...
def _color_gate_on(component: int) -> jnp.ndarray:
    logits = np.full(
        (1, MAX_GRID_SIZE, MAX_GRID_SIZE, 4), -20.0, dtype=np.float32
    )
...
def _decoded(
    ...
    *,
    ...
    colors = color_cell_logits(
        ...
        jnp.zeros(
            (1, MAX_GRID_SIZE, MAX_GRID_SIZE, COLOR_COUNT), dtype=jnp.float32
        )
        if explicit is None
        else explicit,
        _query_grid(grid),
...
def test_colour_decoding_carries_a_batch_axis_independently() -> None:
    ...
    colors = color_cell_logits(
        ...
        jnp.zeros(
            (2, MAX_GRID_SIZE, MAX_GRID_SIZE, COLOR_COUNT), dtype=jnp.float32
        ),
        grids,

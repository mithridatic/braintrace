"""Run the sealed V42 task-gated PP-prop capability oracle."""
...
class ExpertOracleConfig:
    """Configure one sealed V42 synthetic capability oracle.
    ...
    synthetic_seed: int = 21108
    oracle_task_count: int = 120
    oracle_seed: int = 92108
...
def run_expert_oracle(config: ExpertOracleConfig) -> dict[str, object]:
    """Train and score one sealed V42 synthetic capability oracle.

    Parameters
    ----------
    config : ExpertOracleConfig
    ...
    if not isinstance(config, ExpertOracleConfig):
        raise TypeError("config must be an ExpertOracleConfig instance.")
    ...
    with jax.default_device(device):
        ...
        model = TaskGatedOnlineRNN(model_config)
        ...
        for chunk_index in range(chunk_count):
            ...
            print(
                f"[task-gated] chunk={chunk_index + 1}/{chunk_count} "
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--synthetic-seed", type=int, default=21108)
    parser.add_argument("--oracle-task-count", type=int, default=120)
    parser.add_argument("--oracle-seed", type=int, default=92108)
...
def main(argv: list[str] | None = None) -> int:
    """Run the V42 capability oracle and print its strict gate score.
    ...
    result = run_expert_oracle(ExpertOracleConfig(**vars(args)))
    print(
        msgspec.json.encode(
            {

"""Run the sealed V41 continuous spatial PP-prop capability oracle."""
...
from examples.pp_prop.latent_workspace_continuous_spatial_training import (
    ContinuousSpatialPPPropTrainer,
    continuous_spatial_parameter_arrays,
    continuous_spatial_parameter_digest,
    continuous_spatial_parameter_leaf_arrays,
    evaluate_continuous_spatial_model,
    save_continuous_spatial_checkpoint,
)
...
class ContinuousSpatialOracleConfig:
    """Configure one sealed V41 synthetic capability oracle.
    ...
    def __post_init__(self) -> None:
        ...
        model_config = ContinuousSpatialConfig(
...
def _parameter_leaves_finite(model: ContinuousSpatialARC) -> bool:
    return all(
...
def run_continuous_spatial_oracle(
    config: ContinuousSpatialOracleConfig,
) -> dict[str, object]:
    """Train and score one sealed V41 synthetic capability oracle.
    ...
    if not isinstance(config, ContinuousSpatialOracleConfig):
        raise TypeError("config must be a ContinuousSpatialOracleConfig instance.")
    ...
    with jax.default_device(device):
        ...
        model = ContinuousSpatialARC(model_config)
        parameter_before = continuous_spatial_parameter_digest(model)
        groups_before = continuous_spatial_parameter_arrays(model)
        leaves_before = continuous_spatial_parameter_leaf_arrays(model)
        evaluation_before = evaluate_continuous_spatial_model(
            model,
            oracle_episodes,
            trace_decay=config.trace_decay,
            batch_size=config.training_batch_size,
        )
        trainer = ContinuousSpatialPPPropTrainer(
            model,
            batch_size=config.training_batch_size,
        ...
        for chunk_index in range(chunk_count):
            ...
            print(
                f"[continuous-spatial] chunk={chunk_index + 1}/{chunk_count} "
                f"loss={observed[-1]:.6f}",
                flush=True,
            )
        training_seconds = time.perf_counter() - started
        parameter_after = continuous_spatial_parameter_digest(model)
        groups_after = continuous_spatial_parameter_arrays(model)
        leaves_after = continuous_spatial_parameter_leaf_arrays(model)
        evaluation = evaluate_continuous_spatial_model(
            model,
            oracle_episodes,
            trace_decay=config.trace_decay,
            batch_size=config.training_batch_size,
        ...
        checkpoint_digest = save_continuous_spatial_checkpoint(
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--training-updates", type=int, default=600)
    ...
    parser.add_argument("--synthetic-seed", type=int, default=20108)
    parser.add_argument("--oracle-task-count", type=int, default=120)
    parser.add_argument("--oracle-seed", type=int, default=82108)
...
def main(argv: list[str] | None = None) -> int:
    """Run the V41 capability oracle and print its strict gate score.
    ...
    result = run_continuous_spatial_oracle(
        ContinuousSpatialOracleConfig(**vars(args))
    )
    print(
        msgspec.json.encode(
            {

...
| Hard: firing range | rheobase from the ramp; count in the step run at the primary drive; last spike current on the ramp; block current | highest still-firing long-square amplitude (`long-square-index.json` / NWB stimulus, LJP-corrected) | none beyond the recorded range | `recruitable_in_human_range`: ramp rheobase at or below the human's highest recorded amplitude and the step run at the primary drive fires at least once (the step measurement the campaign has); `fires_at_highest`: still fires at the human's highest amplitude; `no_block_in_recorded_range`: no depolarisation block at or below it; the block edge above it is recorded, not scored |
| Recorded, not a rule: `rheobase_in_step` | 1 s ramp rheobase (570 / 270 / 570 / 300 pA/s) | human long-square rheobase; beside it the donor's Allen `threshold_i_ramp` (Allen's own slow ramp, rheobase 3-10 s after onset) and the documented step-to-ramp offset: L2 200 -> 236 (+36), L4 50 -> 52 (+2), PV 120 -> 192 (+72), SST 50 -> 77 (+27) pA (`human-datums.json` `allen_ramp_threshold`, pull `.cache/h01/allen-donor-ephys-features.json` a701d082...) | one sweep step, written as a reading | a ramp rheobase and a step rheobase are different measurement chains (coordinator, 2026-09-16, flagged to J): the reading is recorded with both human thresholds and is not scored |

...
def build_nodes():
    return [
        ...
        n("datum_fire", D, "Datum, firing range: the human sweep set's lowest firing and highest still-firing long-square amplitude per donor (long-square-index.json / NWB stimulus, LJP-corrected); tolerance one human sweep step. Model reading: rheobase from the ramp; still fires at the human's highest amplitude; count at the primary input inside the human count band. Count band (human-datums.json count_band, corrected 2026-09-16 before any run): min to max of the counts of every long-square sweep whose amplitude lies within one sweep_step_pa of primary_pa, inclusive, the primary's own repeats included, because a single sweep at one amplitude carries no repeat spread; the same one-step-of-drive tolerance the rheobase rule uses. L2 290/310/330 pA -> [9, 12] (sweeps 52-54); L4 90/110 pA -> [12, 17] (39-40; the family has no 70 pA sweep); PV 170/190/210 pA -> [4, 23] (34-36); SST 90/100 x4/110 pA -> [12, 21] (32, 44-47, 33).",
          "Qualification rule: a dose outside the recorded cell's range measures the model, not the difference from the recording, so the block edge above the human range is recorded and not scored. At rheobase the human repeats give 0 or 1; the information there is the rheobase and the level, not a count.", [DATUMS, CM, SPEC], "unverified"),
        n("datum_rest", D, "Datum, rest: donor rest across the long-square family (human-datums.json rest_repeat_mean_mv: L2 -84.01, L4 -80.60, PV -87.12, SST -77.50 mV); tolerance 3 sd of the donor's repeat level, the across-sweep sd of the same family (rest_repeat_sd_mv: L2 0.31, L4 0.47, PV 0.38, SST 0.39 mV; bands 0.92, 1.42, 1.15, 1.16 mV), on the 100 ms before the pulse. Return leg (corrected 2026-09-16 before any run: it had been scored against the pre-pulse datum): the family's mean over the 10 ms ending 200 ms after the pulse offset, the exact window the transfer runner scores as return_mv (human-datums.json after_repeat_mean_mv: L2 -84.94, L4 -81.03, PV -87.18, SST -79.18 mV; tolerance 3 x after_repeat_sd_mv: L2 0.96, L4 1.83, PV 0.45, SST 2.27 mV; bands 2.87, 5.49, 1.34, 6.82 mV). Every sweep of all four families reaches that window; a leg the recording does not reach is not scored. Datum and tolerance of each leg come from one repeat set over that leg's own window; the single-sweep donor-rest.json value (L2 -84.26, L4 -81.01, PV -86.76, SST -77.15 mV) is kept beside it for the legacy verdict. The within-trace sd of one sweep (0.04-0.15 mV) is not the repeat level and is only the fallback where no repeat sd is recorded. Failure: any -20 mV crossing before the pulse.",
          "The former 10 mV band and the max(2, 30 percent) count band are recorded beside the new readings for comparison and are not the gate.", [DATUMS, REST, SPEC], "unverified"),

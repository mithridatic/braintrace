...
def build_nodes():
    return [
        ...
        n("datum_rest", D, "Datum, rest: donor rest from donor-rest.json (L2 -84.26, L4 -81.01, PV -86.76, SST -77.15 mV); tolerance 3 sd of the donor's repeat level, the across-sweep sd of the pre-pulse rest over the long-square family (human-datums.json rest_repeat_sd_mv: L2 0.31, L4 0.47, PV 0.38, SST 0.39 mV; bands 0.92, 1.42, 1.15, 1.16 mV), on the 100 ms before the pulse and the 10 ms ending 200 ms after it. The within-trace sd of one sweep (0.04-0.15 mV) is not the repeat level and is only the fallback where no repeat sd is recorded. Failure: any -20 mV crossing before the pulse.",

...
def build_nodes():
    return [
        ...
        n("datum_fire", D, "Datum, firing range: the human sweep set's lowest firing and highest still-firing long-square amplitude per donor (long-square-index.json / NWB stimulus, LJP-corrected); tolerance one human sweep step. Model reading: rheobase from the ramp; still fires at the human's highest amplitude; count at the primary input inside the human repeat range.",
          "Qualification rule: a dose outside the recorded cell's range measures the model, not the difference from the recording, so the block edge above the human range is recorded and not scored. At rheobase the human repeats give 0 or 1; the information there is the rheobase and the level, not a count.", [CM, SPEC], "unverified"),
        n("datum_rest", D, "Datum, rest: donor rest across the long-square family (human-datums.json rest_repeat_mean_mv: L2 -84.01, L4 -80.60, PV -87.12, SST -77.50 mV); tolerance 3 sd of the donor's repeat level, the across-sweep sd of the same family (rest_repeat_sd_mv: L2 0.31, L4 0.47, PV 0.38, SST 0.39 mV; bands 0.92, 1.42, 1.15, 1.16 mV), on the 100 ms before the pulse and the 10 ms ending 200 ms after it. Datum and tolerance come from one repeat set; the single-sweep donor-rest.json value (L2 -84.26, L4 -81.01, PV -86.76, SST -77.15 mV) is kept beside it for the legacy verdict. The within-trace sd of one sweep (0.04-0.15 mV) is not the repeat level and is only the fallback where no repeat sd is recorded. Failure: any -20 mV crossing before the pulse.",
          "The former 10 mV band and the max(2, 30 percent) count band are recorded beside the new readings for comparison and are not the gate.", [REST, SPEC], "unverified"),

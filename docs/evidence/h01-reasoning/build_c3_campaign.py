...
def build_nodes():
    return [
        ...
        n("datum_fire", D, "Hard failure edges (J, 2026-09-16, gate split while the chains ran; runs unchanged): finite traces; no -20 mV crossing before the pulse; firing range from the human sweep set's lowest firing and highest still-firing long-square amplitude per donor (long-square-index.json / NWB stimulus, LJP-corrected), tolerance one human sweep step: the ramp rheobase within the step, the ramp still firing at the human's highest amplitude, and no depolarisation block at or below it; the dt-half repeat reproducing the count. "

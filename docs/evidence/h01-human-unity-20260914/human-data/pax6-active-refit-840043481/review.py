...
from docs.evidence.h01_pax6_bessel_current import prepare_commands, predict_active
...
assert sha(repo / 'docs/evidence/h01_pax6_bessel_current.py') == candidate['model_source_sha256']
assert sha(repo / 'docs/specs/2026-09-14-h01-pax6-active-refit.md') == candidate['spec_sha256']
...
with np.load(out / 'training-source.npz') as source:
    raw = dict(source)
...
decision = dict(status='ready for frozen prospective predictions' if prerequisite else 'rejected before new response access',
                ...
                full_rmse_pa=rms(full['residual_pa']), passive_rmse_pa=rms(full['passive_pa']-observed),
...
for name, selected in [('depolarization', [70, 74, 78]), ('conditioning', [89, 93, 97]),
    ...
    for panels, sweep in zip(axes, selected):
        ...
        panels[1].plot(t, full['filtered_pa'][mask], lw=.8, label='Candidate')
...
for panels, sweep in zip(axes, [123, 132, 141]):
    ...
    for ax, segment in zip(panels, transitions[:3]):
        ...
        ax.plot(times[mask]-onset, full['filtered_pa'][mask], lw=.9, label='Candidate')

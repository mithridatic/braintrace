...
electrical_result = dict(per_sweep=electrical_errors, validation_reference_rmse_mv=e_ref,
                         validation_candidate_rmse_mv=e_error, failed_sweeps=e_failures,
                         passed=bool(electrical_pass))
(OUT/'electrical-result.json').write_text(json.dumps(electrical_result, indent=2)+'\n', encoding='utf-8')
np.savez_compressed(OUT/'electrical-observations.npz', **stored)
records, extras = {}, {}
stored['slot_is_recorded_spike'] = np.array([True, True, True, True, False, True])
epoch_fields = {key: getattr(tables[3].epochWaveformsBySweep[0], key)
                for key in ['p1s', 'p2s', 'types', 'levels', 'pulsePeriods', 'pulseWidths']}
(OUT/'presynaptic-epochs.json').write_text(json.dumps(epoch_fields, indent=2)+'\n', encoding='utf-8')
...
def extract(sweep):
    ...
    abf.setSweep(sweep, channel=1)
    epochs = tables[3].epochWaveformsBySweep[sweep]
    assert all(getattr(epochs, key) == values for key, values in epoch_fields.items())
    # Validate the complete source clock and both voltage channels, retaining every crossing.
    all_events = spike_windows(clock, pre, abf.sweepY, rate_hz=abf.dataRate)
    assigned = all_events['crossing_indices']
    assert len(assigned) == 5 and not len(all_events['boundary_crossings'])
    starts = np.array([28125, 38125, 48125, 58125, 93225])
    assert np.all((assigned >= starts) & (assigned < starts+150))
    unassigned = np.array([], dtype=int)
    extras[sweep] = []
    anchors = np.insert(assigned, 4, 68125)
    indices = anchors[:, None]+np.arange(-500, 2501)
    assert indices.min() >= 0 and indices.max() < clock.size
    events = dict(indices=indices, time_s=clock[indices], pre_mv=pre[indices],
                  post_mv=abf.sweepY[indices].copy(), accepted_crossings=assigned,
...
slot_gate = strength['interior'] and error <= .75*reference and not failures
actual = stored['slot_is_recorded_spike']
a_ref = float(np.sqrt(np.mean(obs[15:, actual]**2)))
a_error = float(np.sqrt(np.mean((obs[15:, actual]-pred[15:, actual])**2)))
a_ratios = np.sqrt(np.mean((obs[15:, actual]-pred[15:, actual])**2, axis=(1, 2)))/np.sqrt(np.mean(obs[15:, actual]**2, axis=(1, 2)))
a_failures = (np.flatnonzero(a_ratios > 1.05)+15).tolist()
actual_gate = strength['interior'] and a_error <= .75*a_ref and not a_failures
synaptic_pass = slot_gate and actual_gate
actual_result = dict(validation_reference_rmse_mv=a_ref, validation_candidate_rmse_mv=a_error,
                     validation_sweep_error_ratios=a_ratios.tolist(), failed_sweeps=a_failures,
                     passed=bool(actual_gate))
...
result = dict(candidate=candidate, candidate_sha256=frozen_sha,
              ...
              actual_spike_windows=actual_result,
              absent_spike_slot=4, absent_slot_anchor_index=68125,
              all_six_slots_passed=bool(slot_gate),
              command_truncation_semantics='unresolved; model driven by recorded spikes',

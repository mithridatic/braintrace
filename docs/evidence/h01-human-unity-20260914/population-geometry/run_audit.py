...
for identity in sorted(reference['cells'], key=int):
    ...
    for label, region in regions.items():
        intervals = region.evaluate(imported.morphology).intervals
        assert np.allclose(np.asarray(intervals).reshape(-1,3), np.asarray(saved['electrical_intervals'][label]).reshape(-1,3),rtol=0,atol=1e-12)
    ...
    np.savez_compressed(path, source_rows=rows,source_edges_um=expected, imported_edges_um=actual,
          ...
          source_branch_area_um2_length_um=np.asarray(branch_sources), cv_observations=cv_data)
    report.update(status='passed', cell_id=identity, component=component,source_sha256=imported.source_sha256,
...
write(root/'result.json',dict(status='passed',cells=reports,cell_count=len(reports),

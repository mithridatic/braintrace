...
| Human acceptance | Energetic search 2026-09-06 ([isolation](h01-isolation-decision.md), [multivari](h01-matryoshka2-decision.md)): I finalist reproduces the loop and trough at three inputs within repeatability limits and predicted 9 of 11 holdout rows, but the 0.23 nA holdout FAILS the approved contract on count and first peak ([I result](h01-i-energetic-result.md), [prediction](h01-prediction-i.md)); E is a perisomatic fit whose rise is load-limited, structural FAIL, sweep 53 holdout fails the contract on count and peaks ([E result](h01-e-energetic-result.md), [prediction](h01-prediction-e.md)). Both holdouts are now spent. | I: post-trough burst and accommodation, one evaluation in reserve; finalist stays experimental until a transfer gate. E: family change (axonal initiation site) returned to the user |

...
import numpy as np
from h01_e_current_paths import PLATEAU_WINDOWS, _window_stats, cycle_summary, load_run
...
def main(argv=None):
    ...
    human_input = sc.add_mutually_exclusive_group(required=True)
    human_input.add_argument("--human-root", type=Path, help="Legacy root containing .cache/human-pyramidal-l2.")
    human_input.add_argument("--human-npz", type=Path, help="Explicit human sweep export; no cache-layout assumption.")
    ...
    human_path = args.human_npz if args.human_npz else args.human_root/HUMAN_SWEEP56
    ...
    direct = write_bundle([args.folder/f"{c['name']}-sweep56" for c in candidates],
                          args.folder/f"{args.output_stem}-direct", [human_path])
    decision = decide(args.folder, candidates, human_path)

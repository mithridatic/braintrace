...
**Verdict: PASS on the claimed rows, FAIL on the train.** The elemental loop and
the trough are explained and predicted on the closed holdout; the early burst,
the count and the accommodation along the train are not explained and are
...
flowchart TD
    ...
    P --> P1[claimed rows pass; count, burst, accommodation fail as predicted]
```
...
| P | Does the explanation predict the closed holdout? | Yes on every claimed row; the three rows predicted to fail failed; the late fall rate failed unpredicted. | [prediction](h01-prediction-i.md) |

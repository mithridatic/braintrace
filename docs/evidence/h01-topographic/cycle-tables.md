# SP15 stage 6: the cyclical family, cycle by cycle

Retained traces only; thresholds and rises by the campaign's `landmarks()` on a uniform grid.
Columns: spike number, threshold (mV), maximum rise (V/s), time from pulse onset (ms).

## E human

**sweep 48** (1 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -55.6 | 345 | 102 |

**sweep 49** (3 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -56.2 | 345 | 74 |
| 2 | -54.7 | 319 | 248 |
| 3 | -54.7 | 320 | 981 |

step at spike 2: +1.5 mV, rise x0.93, 174 ms after spike 1; last spike +1.5 mV, rise x0.93

**sweep 50** (5 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -55.8 | 351 | 58 |
| 2 | -54.6 | 315 | 91 |
| 3 | -54.4 | 314 | 312 |
| 4 | -54.1 | 312 | 619 |
| 5 | -54.3 | 308 | 893 |

step at spike 2: +1.2 mV, rise x0.90, 34 ms after spike 1; last spike +1.5 mV, rise x0.88

**sweep 51** (7 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -57.3 | 348 | 45 |
| 2 | -53.7 | 313 | 67 |
| 3 | -54.3 | 312 | 199 |
| 4 | -54.1 | 306 | 356 |
| 5 | -54.8 | 305 | 525 |
| 6 | -54.3 | 302 | 709 |
| 7 | -53.7 | 300 | 894 |

step at spike 2: +3.6 mV, rise x0.90, 22 ms after spike 1; last spike +3.5 mV, rise x0.86

**sweep 52** (9 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -56.6 | 352 | 40 |
| 2 | -53.5 | 306 | 57 |
| 3 | -53.7 | 304 | 145 |
| 4 | -53.6 | 303 | 272 |
| 5 | -54.1 | 298 | 400 |
| 6 | -54.1 | 297 | 522 |
| 7 | -53.8 | 291 | 654 |
| 8 | -53.8 | 293 | 786 |
| 9 | -54.0 | 291 | 931 |

step at spike 2: +3.1 mV, rise x0.87, 17 ms after spike 1; last spike +2.6 mV, rise x0.83

**sweep 53** (10 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -56.4 | 348 | 36 |
| 2 | -55.0 | 301 | 47 |
| 3 | -53.9 | 304 | 108 |
| 4 | -53.3 | 304 | 218 |
| 5 | -52.8 | 299 | 333 |
| 6 | -52.9 | 294 | 453 |
| 7 | -53.4 | 296 | 575 |
| 8 | -52.9 | 289 | 691 |
| 9 | -53.2 | 289 | 815 |
| 10 | -54.3 | 288 | 935 |

step at spike 2: +1.4 mV, rise x0.87, 11 ms after spike 1; last spike +2.1 mV, rise x0.83

**sweep 55** (13 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -57.2 | 350 | 28 |
| 2 | -54.9 | 296 | 39 |
| 3 | -52.6 | 283 | 62 |
| 4 | -53.7 | 297 | 140 |
| 5 | -53.5 | 291 | 220 |
| 6 | -53.5 | 291 | 306 |
| 7 | -53.5 | 288 | 393 |
| 8 | -53.4 | 288 | 480 |
| 9 | -53.3 | 283 | 566 |
| 10 | -52.6 | 280 | 659 |
| 11 | -52.9 | 284 | 749 |
| 12 | -53.4 | 276 | 839 |
| 13 | -52.9 | 279 | 932 |

step at spike 2: +2.2 mV, rise x0.85, 11 ms after spike 1; last spike +4.3 mV, rise x0.80

**sweep 56** (1 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -54.7 | 332 | 205 |

**sweep 57**: no spike

**sweep 58**: no spike

**sweep 59** (1 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -55.2 | 328 | 208 |

**sweep 60** (1 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -54.8 | 330 | 152 |

**sweep 61** (1 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -54.5 | 329 | 198 |

**sweep 62** (1 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -55.0 | 329 | 203 |

## E fit

**200 pA** (4 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -57.2 | 598 | 127 |
| 2 | -57.0 | 603 | 196 |
| 3 | -57.1 | 608 | 456 |
| 4 | -57.0 | 599 | 752 |

step at spike 2: +0.2 mV, rise x1.01, 69 ms after spike 1; last spike +0.2 mV, rise x1.00

**250 pA** (8 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -57.0 | 619 | 55 |
| 2 | -57.1 | 620 | 76 |
| 3 | -57.0 | 597 | 122 |
| 4 | -57.1 | 611 | 306 |
| 5 | -57.1 | 610 | 478 |
| 6 | -57.2 | 604 | 643 |
| 7 | -57.1 | 609 | 803 |
| 8 | -57.2 | 599 | 961 |

step at spike 2: -0.1 mV, rise x1.00, 21 ms after spike 1; last spike -0.2 mV, rise x0.97

**310 pA** (10 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -57.2 | 639 | 34 |
| 2 | -57.2 | 664 | 48 |
| 3 | -57.2 | 619 | 72 |
| 4 | -57.2 | 601 | 193 |
| 5 | -57.2 | 603 | 327 |
| 6 | -57.2 | 600 | 454 |
| 7 | -57.0 | 603 | 577 |
| 8 | -57.1 | 608 | 697 |
| 9 | -57.2 | 609 | 814 |
| 10 | -57.1 | 610 | 930 |

step at spike 2: -0.0 mV, rise x1.04, 14 ms after spike 1; last spike +0.1 mV, rise x0.95

## I human

**0.19 nA** (12 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -60.5 | 597 | 21 |
| 2 | -59.9 | 576 | 28 |
| 3 | -58.5 | 567 | 38 |
| 4 | -56.7 | 540 | 81 |
| 5 | -54.7 | 505 | 149 |
| 6 | -53.7 | 497 | 227 |
| 7 | -53.7 | 494 | 311 |
| 8 | -54.3 | 491 | 407 |
| 9 | -54.2 | 483 | 510 |
| 10 | -51.7 | 445 | 707 |
| 11 | -53.0 | 452 | 824 |
| 12 | -52.0 | 452 | 923 |

step at spike 2: +0.6 mV, rise x0.96, 8 ms after spike 1; last spike +8.5 mV, rise x0.76

**0.27 nA** (43 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -61.7 | 602 | 12 |
| 2 | -60.0 | 570 | 18 |
| 3 | -59.7 | 550 | 24 |
| 4 | -59.6 | 554 | 31 |
| 5 | -57.5 | 550 | 43 |
| 6 | -58.4 | 558 | 65 |
| 7 | -58.2 | 559 | 88 |
| 8 | -58.4 | 555 | 110 |
| 9 | -58.3 | 552 | 133 |
| 10 | -59.0 | 562 | 155 |
| 11 | -58.7 | 555 | 178 |
| 12 | -58.0 | 555 | 202 |
| 13 | -58.3 | 560 | 226 |
| 14 | -58.5 | 557 | 248 |
| ... | | | (29 more) |

step at spike 2: +1.7 mV, rise x0.95, 6 ms after spike 1; last spike +4.8 mV, rise x0.88

**repeat sweep 40** (1 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -58.5 | 577 | 67 |

**repeat sweep 41** (1 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -57.9 | 566 | 84 |

**repeat sweep 42** (1 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -58.3 | 577 | 68 |

**repeat sweep 43** (1 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -57.7 | 570 | 78 |

## I fit

**0.19 nA** (14 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -60.5 | 592 | 28 |
| 2 | -60.5 | 587 | 63 |
| 3 | -60.5 | 597 | 102 |
| 4 | -60.5 | 597 | 147 |
| 5 | -60.5 | 595 | 200 |
| 6 | -60.3 | 588 | 265 |
| 7 | -60.4 | 583 | 339 |
| 8 | -60.5 | 594 | 418 |
| 9 | -60.4 | 594 | 500 |
| 10 | -60.4 | 585 | 582 |
| 11 | -60.3 | 579 | 665 |
| 12 | -60.3 | 586 | 748 |
| 13 | -60.3 | 590 | 832 |
| 14 | -60.3 | 594 | 917 |

step at spike 2: +0.1 mV, rise x0.99, 35 ms after spike 1; last spike +0.2 mV, rise x1.00

**0.27 nA** (37 spikes)

| spike | threshold | rise | t |
| ---: | ---: | ---: | ---: |
| 1 | -61.1 | 613 | 11 |
| 2 | -61.0 | 608 | 27 |
| 3 | -61.0 | 605 | 44 |
| 4 | -60.9 | 603 | 62 |
| 5 | -60.7 | 600 | 81 |
| 6 | -60.7 | 583 | 100 |
| 7 | -60.7 | 582 | 121 |
| 8 | -60.5 | 591 | 143 |
| 9 | -60.4 | 589 | 166 |
| 10 | -60.4 | 582 | 191 |
| 11 | -60.4 | 589 | 217 |
| 12 | -60.3 | 579 | 244 |
| 13 | -60.3 | 576 | 272 |
| 14 | -60.2 | 572 | 300 |
| ... | | | (23 more) |

step at spike 2: +0.1 mV, rise x0.99, 16 ms after spike 1; last spike +0.9 mV, rise x0.93

## Spike 1 across sweeps (recovery between sweeps)

- E human staircase: 7 sweeps; threshold -57.3 to -55.6 mV (sd 0.63); rise 345 to 352 V/s (sd 2.7)
- E human 200 pA repeats: 5 sweeps; threshold -55.2 to -54.5 mV (sd 0.25); rise 328 to 332 V/s (sd 1.5)
- I human repeats: 4 sweeps; threshold -58.5 to -57.7 mV (sd 0.38); rise 566 to 577 V/s (sd 5.3)

## Reading

E human, 230 to 350 pA: the threshold steps up between spike 1 and spike 2, keeps moving through spike 5, then holds for the rest of the second at every drive; the maximum rise steps down at spike 2 by about an eighth and then drifts a further twentieth over the second. Spike 1 is the same in every sweep. At 200 pA the cell fires once and then holds below threshold for the rest of the pulse, and in two of seven repeats it does not fire.
I human: at 0.27 nA the threshold steps up over the first five spikes and then holds; at 0.19 nA it climbs across the whole second as the intervals lengthen. The rise follows the threshold in both.
Both fits hold threshold and rise flat at every drive.
So the use-dependence enters within tens of milliseconds of a spike, does not recover inside the 800 ms hold after a single spike, and has recovered by the next sweep. A mechanism with one time constant of a second accumulates gradually and cannot make the step; SP16 is amended to separate entry from recovery, and its predictions are written per cycle.

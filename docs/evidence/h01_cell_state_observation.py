"""Observe the first invalid state in the unchanged isolated H01 failure."""
...
baseline_path = Path('.cache/h01/readiness/cell7196644737-dt005-traces.npz')
...
with brainstate.environ.context(precision=64):
    network, evidence = make_h01_network(topology, archive, H01Annotations(Path('.cache/h01')),
        include_isolated=True, cells=1, components=components, currents_na={identity:1.}, progress=emit)
    assert evidence['cells'][identity] == reference, 'Cell metadata changed before observation.'
    ...
    with heartbeat('state observation', emit, seconds=30.):
        result = network.run(dt=.005*u.ms, duration=10.*u.ms, spike_recording='population')
    ...
    arrays = {'time_ms':np.asarray(result.time.to_decimal(u.ms))+.005}
    ...
    report = dict(cell_id=identity, scope='unchanged-cell state observation; no physiology pass',
        exact_cell_metadata_match=True, unchanged_geometry=True, n_cv=cell.n_cv,
        ...
        observation_preserves_voltage=delta <= 1e-9 and masks_match,
        ...
        inputs={str(p):hashlib.sha256(p.read_bytes()).hexdigest()
                for p in [reference_path, baseline_path, Path(__file__), traces]})
    args.output.with_suffix('.json').write_text(json.dumps(report, indent=2)+'\n')
    emit(json.dumps(report))
    assert report['observation_preserves_voltage'], 'Probes changed the baseline response.'

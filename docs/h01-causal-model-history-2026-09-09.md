> Historical snapshot of `da552a7` from 2026-09-09. This text includes superseded claims.
> The [current causal model](h01-causal-model.md) controls the present explanation.
> The original document follows without changes.
...
**SP5 stage-1 arms, 2026-09-07 21:58 (observation: none; stopped, untested).** After the addendum registering 1800 s / 3000 s aborts and the 0.8 nA contingency (commit `e2d1291`), the 300 ms `disconnected` control at 0.6 nA was launched at 21:58:08 once the wait condition held (Stage G decision written, docker only `synapse`, no python over 300 MB, CPU 32 percent) and stopped by the user before completion (killed after `Circuit constructed` at 21:59:57, before 22:11:07); no trace, no run-log entry, no cap spent. Arms `measured`, `soma`, `measured-halved` were not launched. Conditions: measured connectivity, closing-restored I wrapper, literature receptor, dt 0.005. JSON: `docs/evidence/h01-ie-inhibition/decision.json`. The registered predictions stay registered-untested; the perisomatic placement hypothesis is **undetermined**; functional inhibition at this pair remains unverified.

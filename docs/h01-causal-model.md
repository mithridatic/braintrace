...
**The onset (provisional).** The recorded cell's spike begins five times more abruptly
than the finalist's: from 10 to 100 V/s it covers 2.7 mV (1.9 to 3.8 over six trains;
...
the campaign to the recorded take-off as a slope, -2.8 mV per decade of approach against
the recording's -3.8 (x2 total held -4.1, x4 -5.0, control -0.1), over ranges that overlap
only between 0.45 and 0.75 mV/ms, where the arm takes off about 1 mV above the recording;
nine spikes at 310 pA against ten. The two x2 arms differ by 0.8 mV, so the site's density
modulates a slide that the coupling makes. The coupling is therefore sufficient to produce
the approach signature at the soma under these conditions, and in every dose tested it also
took the onset outside the recorded band, so this split did not separate the reading of the
spike from the spike itself; it does not name the recorded cell's geometry, only that its
soma reads its site more directly than through the Allen stub. It moves neither the rise, 579 and 655
...
the axon point, 45 um out and firing first, so at or proximal to it, the take-off is
-54.3 to -55.2 mV at every spike, and under the SP16 gate at depth 0.6
...
A single sweep carries no repeat spread, so a count band is read across the sweeps within one step of drive: the counts of every long-square sweep whose amplitude lies within one sweep step of the compared input, the primary's own repeats included (the C3 gate's `count_band`, 2026-09-16). A level is compared with the recording's reading of the same window: the pre-pulse rest with the family's pre-pulse mean, the level 200 ms after the pulse with the family's mean over that window, each with 3 across-sweep sd of its own window; scoring the post-pulse level against the pre-pulse datum was a datum-construction error caught in preflight and corrected before any run.
A plausibility reading (a count at one drive, a resting or post-pulse level) is qualified against the across-cell spread of human cells of the same type, not against one donor's repeat spread: datum = the donor's own value, tolerance = 2 sd across the Allen Cell Types human cells sharing the donor's dendrite type and layer at the same drive (the C3 gate's `type_population`, J 2026-09-16); a failure edge (rheobase, firing at the highest recorded amplitude, block inside the recorded range, a spike before the pulse, a non-finite or non-reproducing trace) keeps its one-sweep-step tolerance. Where the reference table lacks the exact window (Allen `post_vm_mv` is the last 500 ms of the sweep, not 200 ms after offset) the closest documented field is named as the fallback, not hand-picked.
A rheobase is qualified only against a rheobase reached by the same drive: the 1 s ramp rheobase of the C3 gate is not compared with the human long-square rheobase (the donors' own Allen rows put their slow-ramp threshold 2-72 pA above their step threshold), so the failure edge is recruitability inside the recorded range (ramp rheobase at or below the highest recorded amplitude, and firing in the step run at the primary drive), and the ramp-versus-step comparison is written as a reading beside both human thresholds (coordinator 2026-09-16, flagged to J).
